...
function updateCartTable(items, totali) {
    ...
    items.forEach(item => {
        ...
        <tr data-id="${item.id}" data-tipo="${item.tipo}" data-disponibilita="${item.disponibilita}">
            ...
            <td><input type="radio" name="selectedCartItem" value="${item.id}" data-tipo="${item.tipo}"></td>
        </tr>`;
    });
...
// ================== SELEZIONE ITEM PER ACQUISTO ==================
function attachSelectionHandlers() {
    const radios = document.querySelectorAll('input[name="selectedCartItem"]');
    const btnAcquista = document.getElementById("btnAcquista");

    if (!btnAcquista) return;

    radios.forEach(r => {
        r.addEventListener("change", () => {
            btnAcquista.removeAttribute("disabled");
        });
    });
}
